...
const validators = {
	courseValidations: [
		...
		body('topic', 'Unknown course category').trim().toLowerCase().isIn(['software development', 'uiux design', 'cybersecurity', 'cloud computing', 'artificial intelligence']),
		body('level', 'unknown course level').trim().toLowerCase().isIn(['beginner', 'intermediate', 'advanced']),
		// body('sessions', 'Must be array').isArray(),
